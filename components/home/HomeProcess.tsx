    ...
    <>
      <section data-pin-section className="bg-ink text-paper">
        <div data-pin-viewport className="flex min-h-svh flex-col justify-center overflow-hidden py-[clamp(64px,10vh,120px)]">
          ...
          <div
            data-pin-window
            className="w-full snap-x snap-mandatory overflow-x-auto overscroll-x-contain [-ms-overflow-style:none] [scrollbar-width:none] md:overflow-x-hidden md:snap-none [&::-webkit-scrollbar]:hidden"
          >
            <div data-pin-track className="flex w-max gap-[clamp(20px,3vw,40px)] px-[clamp(20px,5vw,56px)]">
              {homeSteps.map((step) => (
                <article key={step.number} className="flex min-h-[clamp(390px,54vh,520px)] w-[clamp(310px,44vw,580px)] shrink-0 snap-center flex-col gap-[18px] rounded-3xl border border-paper/10 bg-ink-soft p-[clamp(28px,3.5vw,52px)] md:snap-align-none">
